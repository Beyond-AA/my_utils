...
def calc_mean_std(feat, eps=1e-5):
    #eps is a small value add to the variance to avoid divid-by-zero
    size = feat.size()
    assert (len(size) == 4)
    N, C = size[:2]
...
def _calc_feat_flatten_mean_std(feat):
    #  takes 3D feat (C, H, W), return mean and of array within channels
...
def coral(source, target):
    ...
    source_f_norm_transfer = torch.mm(
        _mat_sqrt_(target_f_cov_eye), torch.mm(torch.inverse(_mat_sqrt_(source_f_cov_eye)), source_f_norm)
    )
...
if __name__ == '__main__':
    ...
    adain = adaptive_instance_normalization(x, y)
    print(adain)
